        ...
        <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white pt-24 pb-32 relative overflow-hidden">
            ...
            <div className="container mx-auto px-4 max-w-5xl relative z-10">
                    ...
                    <div className="flex flex-col gap-10">
                        {LEVELS.map((level, i) => {
                            ...
                            return (
                                ...
                                >
                                    {/* Level Node */}
                                    <Link href={level.href} className="flex-shrink-0 group">
                                        <motion.div
                                        ...
                                        >
                                            {/* Glow ring */}
                                            <div className={`absolute inset-0 rounded-[40px] ${level.color} opacity-0 group-hover:opacity-20 blur-xl transition-all scale-125`} />

                                            <div className={`w-28 h-28 rounded-[32px] flex items-center justify-center text-white border-b-[8px] shadow-2xl ${level.glow} relative z-10
                                                ${level.color} ${level.border} group-hover:-translate-y-2 transition-all`}>
                                                <level.icon size={44} strokeWidth={2.5} className="drop-shadow-md" />
                                            </div>
                                    ...
                                    <motion.div
                                        whileHover={{ y: -4 }}
                                        className={`flex-1 duo-card bg-white p-6 border-b-[6px] ${level.border} shadow-lg relative overflow-hidden`}
                                    >
                                        {/* Color accent line */}
                                        ...
                                        <div className="flex items-start justify-between mb-4">
                                            <div>
                                                <p className="text-xs font-black uppercase tracking-widest text-duo-gray mb-1">Module {i + 1}</p>
                                                <h3 className={`text-3xl font-black uppercase italic tracking-tighter ${level.textColor}`}>{level.name}</h3>
                                            </div>
                                            <span className={`text-xs font-black px-4 py-2 rounded-full uppercase tracking-wider ${DIFF_COLOR[level.difficulty]}`}>
                                                {level.difficulty}
                                            </span>
                                        </div>

                                        <p className="text-duo-gray font-bold mb-6">{level.desc}</p>

                                        <div className="flex items-center justify-between">
                                            <div className="flex gap-1">
                                                {[...Array(3)].map((_, j) => (
                                                    <Star key={j} size={16} fill="currentColor" className="text-duo-yellow" />
                                                ))}
                                            </div>
                                            <Link href={level.href}>
                                                <motion.button
                                                    ...
                                                    whileTap={{ scale: 0.95 }}
                                                    className={`duo-button ${level.color} text-white font-black text-lg px-8 py-3 flex items-center gap-3 border-b-4 ${level.border}`}
                                                >
                                                    PLAY <ChevronRight size={20} strokeWidth={4} />
                                                </motion.button>
                                            </Link>
                                        </div>
                                    </motion.div>
                ...
                <motion.div
                    ...
                    className="mt-24 duo-card bg-gradient-to-br from-duo-blue to-blue-600 text-white p-12 text-center border-b-[12px] border-blue-800 shadow-2xl"
                >
                    ...
                    <div className="h-4 bg-white/20 rounded-full overflow-hidden mx-4">
                        <motion.div
                            initial={{ width: 0 }} animate={{ width: `${Math.min(100, (xp / 800) * 100)}%` }}
